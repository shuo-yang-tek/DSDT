 ...
 *
 * Disassembly of SSDT-1-SaSsdt.aml, Fri Mar 22 20:13:42 2019
 *
 * Original Table Header:
 *     Signature        "SSDT"
 *     Length           0x0000586C (22636)
 *     Revision         0x02
 *     Checksum         0x8B
 *     OEM ID           "SaSsdt"
 *     OEM Table ID     "SaSsdt "
...
DefinitionBlock ("", "SSDT", 2, "SaSsdt", "SaSsdt ", 0x00003000)
{
    ...
    OperationRegion (SANV, SystemMemory, 0x7AE94D98, 0x0135)
    ...
    Scope (\_SB.PCI0.GFX0)
    {
            ...
            Method (_BCM, 1, NotSerialized)  // _BCM: Brightness Control Method
            {
                If (LAnd (LGreaterEqual (Arg0, Zero), LLessEqual (Arg0, 0x64)))
                {
                    If (And (TCHE, 0x02))
                    {
                        ...
                        If (And (ARDY, One))
                        {
                            Store (Add (Divide (Multiply (Arg0, Subtract (0xFF, And (PFMB, 0xFF))), 0x64, ), And (PFMB, 0xFF)), BCLP)
        ...
        Method (SBLF, 0, NotSerialized)
        {
            If (ARDY)
            {
                If (LAnd (And (TCHE, 0x08), And (PFMB, 0x80000100)))
                {
                    Store (0x08, ASLC)
                    Store (One, ASLE)
                }
    ...
    Field (CMS1, ByteAcc, Lock, Preserve)
    {
        INX1,   8, 
        DAE1,   8
    }
